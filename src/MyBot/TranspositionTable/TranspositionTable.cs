...
public class TranspositionTable
{
    ...
    public void Put(Board board, byte depth, byte flag, int eval, Move? bestMove)
    {
        ...
        if (bucket == null)
        {
            bucket = new TTBucket();
            table[index] = bucket;
            bucket[0] = new TranspositionData(board.ZobristHash, depth, flag, eval, bestMove);
        }
        else
        {
            bucket.Put(board.ZobristHash, depth, flag, eval, bestMove);
        }
...
// 
    ...
    public TranspositionData? Get(Board board)
    {
        ...
        if (bucket != null)
        {
            TranspositionData? entry = bucket.Get(board.ZobristHash);
            if (entry != null)
            {
                return entry;
            }
